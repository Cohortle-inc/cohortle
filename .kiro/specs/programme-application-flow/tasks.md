...
- [x] 9. API routes — organisation page andrepush last commit
 programme template
...
## Addendum: Hardening and Surfacing Tasks (May 2026)

- [x] 30. Fix new user acceptance token handoff
  - [x] 30.1 Update `AuthContext.signup()` to accept `options.skipRedirect` flag
    - When `skipRedirect: true`, set user state but do NOT call `window.location.href`
    - Allows `SignupForm` to perform post-signup actions (token redemption) before navigating
    - File: `cohortle-web/src/lib/contexts/AuthContext.tsx`
    - _Requirements: 14.2, 14.3_
  - [x] 30.2 Update `SignupForm` to pass `skipRedirect: true` when `acceptToken` is present
    - Call `redeemAcceptanceToken` after `signup()` resolves, before any navigation
    - On success: redirect to `/programmes/:id`
    - On failure: redirect to `/dashboard` (graceful fallback)
    - File: `cohortle-web/src/components/auth/SignupForm.tsx`
    - _Requirements: 14.3, 14.4, 14.5_
  - [x] 30.3 Update Login page to show acceptance context when `acceptToken` is in URL
    - Extract `LoginPageInner` client component to read `useSearchParams`
    - Show "Log in to complete your enrolment" heading
    - Pass `prefillEmail` to `LoginForm`
    - File: `cohortle-web/src/app/login/page.tsx`, `cohortle-web/src/app/login/LoginPageInner.tsx`
    - _Requirements: 14.6, 14.7_
  - [x] 30.4 Add `prefillEmail` prop to `LoginForm`
    - File: `cohortle-web/src/components/auth/LoginForm.tsx`
    - _Requirements: 14.6_

- [x] 31. Create `/apply/edit/:id` page
  - [x] 31.1 Create `cohortle-web/src/app/apply/edit/[id]/page.tsx`
    - Load application via `getApplication(appId)` — redirect to dashboard if not `draft`
    - Load form template via `getApplicationForm(programme_id)`
    - Pre-populate form with existing `responses`
    - On save: call `PUT /v1/api/applications/:id` with updated responses
    - File: `cohortle-web/src/app/apply/edit/[id]/page.tsx`
    - _Requirements: 15.1, 15.2, 15.3, 15.4_
  - [x] 31.2 Update `ApplicationForm` to accept `initialResponses` and `submitLabel` props
    - File: `cohortle-web/src/components/application/ApplicationForm.tsx`
    - _Requirements: 15.2_

- [x] 32. Replace rejection `window.prompt()` with modal
  - [x] 32.1 Add `rejectingAppId` and `rejectionReason` state to applications list page
  - [x] 32.2 Replace `window.prompt()` call with modal trigger
  - [x] 32.3 Add rejection modal with textarea, validation, and confirm/cancel buttons
    - File: `cohortle-web/src/app/convener/programmes/[id]/applications/page.tsx`
    - _Requirements: 16.1, 16.2, 16.3, 16.4_

- [x] 33. Fix cohort loading on application detail page
  - [x] 33.1 Load cohorts in parallel with application data using `getCohorts(programmeId)`
  - [x] 33.2 Populate cohort selector from loaded cohorts state (not `app.cohorts`)
    - File: `cohortle-web/src/app/convener/programmes/[id]/applications/[appId]/page.tsx`
