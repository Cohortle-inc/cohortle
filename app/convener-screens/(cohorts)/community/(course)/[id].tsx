...
const Index = () => {
  ...
  const { data: modules = [], isLoading: modulesLoading } = useGetModules(numeric);
...
export const Module = ({ id, community_id, title: initialTitle }: ModuleType) => {
  ...
  const saveModuleTitle = () => {
    if (title.trim() && title !== initialTitle) {
      editModule({ community_id, module_id: id, data: { title: title.trim() } });
    }
    setIsEditing(false);
  };
  ...
  const handleStatusChange = (lessonId: number, status: 'published' | 'draft') => {
    editLesson(
  ...
  const handleRenameLesson = () => {
    ...
    editLesson(
      { module_id: id, lesson_id: selectedLesson.id, data: { name: newLessonName.trim() } },
      { onSuccess: () => { refetch(); setOptionModal(0); } }
    );
  };
  ...
  .map((lesson: LessonProp) => (
    <View key={lesson.id} style={styles.lessonRow}>
      ...
      >
        ...
        {lessons.length === 0 && <Text style={styles.noLessons}>No lessons yet</Text>}
      </View>

      {/* Module Menu */}
      <OptionModal isVisible={menuOpen} onBackdropPress={() => setMenuOpen(false)}>
        <View style={styles.menu}>
          <Text style={styles.menuTitle}>{title}</Text>
          <TouchableOpacity onPress={() => { setIsEditing(true); setMenuOpen(false); }}>
            <Text style={styles.menuItemText}>Rename Module</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => { setMenuOpen(false); setShowDeleteConfirm(true); }}>
            <Text style={[styles.menuItemText, { color: '#EE3D3E' }]}>Delete Module</Text>
          </TouchableOpacity>
        </View>
      </OptionModal>

      {/* Lesson Options Modal */}
      <OptionModal isVisible={lessonModalOpen} onBackdropPress={() => setLessonModalOpen(false)}>
          ...
          {selectedLesson && (
            <>
              ...
              <Dropdown
                ...
                onChange={(val) => handleStatusChange(selectedLesson.id, val as any)}
              />
              ...
              <TouchableOpacity
                onPress={() => { setOptionModal(4); setLessonModalOpen(false); }}>
                <Text >Rename Lesson</Text>
              </TouchableOpacity>

              <TouchableOpacity onPress={() => { setOptionModal(5); setLessonModalOpen(false); }}>
      ...
      <SlideModal isVisible={optionModal !== 0} onBackdropPress={() => setOptionModal(0)}>
        {optionModal === 4 && selectedLesson && (
          <View style={styles.formModal}>
            ...
            <TextInput
              ...
              autoFocus
            />
            <TouchableOpacity style={styles.primaryBtn} onPress={handleRenameLesson}>
        ...
        {optionModal === 5 && selectedLesson && (
          <View style={styles.formModal}>
            ...
            <TouchableOpacity style={styles.dangerBtn} onPress={handleDeleteLesson}>
              <Text style={styles.btnText}>Delete Lesson</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.cancelBtn} onPress={() => setOptionModal(0)}>
      ...
      <SlideModal isVisible={showDeleteConfirm} onBackdropPress={() => setShowDeleteConfirm(false)}>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#fff', paddingVertical: 16, paddingHorizontal: 8 },
  header: { marginBottom: 20 },
  title: { fontSize: 24, fontWeight: '700' },
  courseType: { fontSize: 14, color: '#666', marginTop: 4 },
  link: { color: '#391D65', textDecorationLine: 'underline' },
  topInfo: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 },
  topInfoText: { fontSize: 16 },
  bold: { fontWeight: '700' },
  addButton: { backgroundColor: '#4B2E83', paddingHorizontal: 18, paddingVertical: 10, borderRadius: 24 },
  addButtonText: { color: '#fff', fontWeight: '600' },
  modulesContainer: { marginTop: 10 },
  sectionTitle: { fontSize: 14, color: '#8E8E8E', fontWeight: '600', marginBottom: 16 },
  ...
  titleInput: { flex: 1, fontSize: 18, fontWeight: '700', backgroundColor: '#f5f5f5', padding: 10, borderRadius: 8 },
  addNew: { backgroundColor: '#F5F2FB', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20 },
  ...
  menu: { backgroundColor: 'white', padding: 28, borderTopEndRadius: 16, borderTopStartRadius: 16, gap: 20 },
  menuTitle: { fontSize: 18, fontWeight: '600', textAlign: 'center' },
  menuItemText: { fontSize: 16, textAlign: 'center' },

  lessonMenu: { backgroundColor: 'white', padding: 24, borderTopStartRadius: 16, borderTopEndRadius: 16, gap: 20 },
  ...
  formModal: { backgroundColor: 'white', padding: 32, borderRadius: 20, marginHorizontal: 20, gap: 20 },
  formTitle: { fontSize: 22, fontWeight: '700', textAlign: 'center' },
  formMessage: { fontSize: 15, color: '#666', textAlign: 'center' },
  input: { borderWidth: 1, borderColor: '#ddd', padding: 14, borderRadius: 12, fontSize: 16 },
  ...
  cancelText: { color: '#391D65', textAlign: 'center', fontWeight: '600', fontSize: 16 },

  deleteModal: { backgroundColor: 'white', padding: 32, borderRadius: 20, alignItems: 'center', marginHorizontal: 20 },
  deleteTitle: { fontSize: 18, fontWeight: '700', marginTop: 20, textAlign: 'center' },
  deleteMessage: { fontSize: 16, color: '#666', textAlign: 'center', lineHeight: 24, marginVertical: 16 },
});
