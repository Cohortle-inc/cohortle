...
// Make Bunny CDN optional - log warning instead of throwing error
const BUNNY_ENABLED = !!(BUNNY_LIBRARY_ID && BUNNY_ACCESS_KEY);
if (!BUNNY_ENABLED) {
  console.warn("⚠️  Bunny CDN not configured (BUNNY_LIBRARY_ID or BUNNY_ACCESS_KEY missing). Video upload features will be disabled.");
}
...
// Use system temp directory for uploads to avoid permission issues (EACCES)
const uploadDir = os.tmpdir();

// Multer: Store file on disk temporarily
...
async function uploadToBunny(filePath, options = {}) {
  if (!BUNNY_ENABLED) {
    throw new Error("Bunny CDN is not configured. Please set BUNNY_LIBRARY_ID and BUNNY_ACCESS_KEY environment variables.");
  }
  ...
  try {
    ...
    // Delete temp file after upload (success or failure)
...
function getPlaybackUrl(guid) {
  if (!BUNNY_ENABLED || !guid) return null;
